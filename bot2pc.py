...
def UsersLoop(login):
    while True:
        if IsSessionEnded():
            # Użycie nowego selektora do odczytania wyników sesji
            session_result = browser.find_element(By.ID, 'finish_page').get_attribute('innerHTML')
            send_session_summary(session_result, login)
        ...
        try:
            ...
            if not addToArray:
                ...
                send_discord_message(f"'**{polishSentence}**' jako '**{englishWord}**' ✅")  # Zmiana na 'jako'
            time.sleep(0.5)
...
def send_session_summary(session_result, login):
    # Parsowanie wyników sesji
    lines = session_result.split("<br>")
    days_worked = ""
    words_summary = ""

    for line in lines:
        line = line.strip()
        if line.startswith("Dni pracy w tym tygodniu:"):
            days_worked = line.split(":")[1].strip()
        elif line.startswith("Powtórzyłeś"):
            words_summary = line.split(",")[1].strip()
            words_summary = words_summary.split(" ")[0] + "/" + words_summary.split(" ")[1]

    send_discord_message(f"Zakończenie sesji {login}. Powtórzyłeś {words_summary}. Dni: {days_worked}.")
